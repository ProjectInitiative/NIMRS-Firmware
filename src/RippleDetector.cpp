...
void RippleDetector::processBuffer(float* data, size_t len, float sampleRate) {
    if (len == 0 || sampleRate <= 0.0f) return;
...
float RippleDetector::getFrequency() const {
    ...
    // Let's just return current freq. The caller handles timeout if needed (by checking timestamp?)
    // But here we don't have global time.
    // Ideally we should decay it if _samplesSincePulse is huge.
    // For now, let's leave it as last detected freq.
    return _currentFreq;
}
