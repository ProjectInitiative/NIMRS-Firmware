...
pub fn setup() {
    ...
    decoder::init();
    log::info!("DCC: decoder started on pin {}", pinout::TRACK_LEFT_3V3);
}

pub fn loop_once() {
    // DCC is interrupt-driven; nothing to poll
}
...
pub extern "C" fn notifyDccFunc(_addr: u16, _addr_type: u8, func_grp: u8, func_state: u8) {
    ...
    let mut state = SYSTEM_STATE.lock().unwrap();
    if func_grp == 0 {
        state.functions[0] = (func_state & 0x10) != 0;
        state.functions[1] = (func_state & 0x01) != 0;
        state.functions[2] = (func_state & 0x02) != 0;
        state.functions[3] = (func_state & 0x04) != 0;
        state.functions[4] = (func_state & 0x08) != 0;
    } else {
        for i in 0..8 {
