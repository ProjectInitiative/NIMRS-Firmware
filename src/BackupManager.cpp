...
void BackupManager::generateBackup(Print* output) {
    ...
    if (root && root.isDirectory()) {
        ...
        while (file) {
            ...
            // Ensure relative path (no leading slash for tar standard usually, but LittleFS has them)
    ...
    for (int i = 0; i < 1024; i++) output->write(0);
}

void BackupManager::writeTarHeader(Print* out, const String& filename, size_t size) {
    uint8_t header[512];
    memset(header, 0, 512);
...
void BackupManager::writePadding(Print* out, size_t size) {
    ...
    for (size_t i = 0; i < padding; i++) out->write(0);
}
...
String BackupManager::createConfigJson() {
    ...
    // ESP32 stores WiFi in NVS under "nvs.net80211" usually, but not easily accessible via API.
    // However, ConnectivityManager stores them? No, ConnectivityManager just calls WiFi.begin().
    // Wait, check ConnectivityManager.cpp again.
    // It calls `WiFi.begin()`. If params are not passed, it uses SDK storage.
    // If `handleWifiSave` is called, it calls `WiFi.begin(ssid, pass)`.
    // We cannot retrieve the password from the SDK.
    ...
    // So we can only backup the SSID if connected. The password is lost to us (security).
    // Unless we store them ourselves.
    // Strategy: We will backup what we can. If we can't get the password, we leave it blank.
    // The user will have to re-enter WiFi if they restore to a wiped chip.
    // BUT, `doc["wifi_ssid"]` is useful.
    ...
    while (pos < len) {
        if (_state == READ_HEADER) {
            ...
            if (_headerPos == 512) {
                ...
                bool isEmpty = true;
                for (int i=0; i<512; i++) {
                    if (_headerBuf[i] != 0) { isEmpty = false; break; }
                }
                ...
                if (_currentFileSize == 0) {
                     ...
                     _state = READ_HEADER; // No padding for 0 size? Wait, tar spec says data is 512 aligned.
...
bool BackupManager::applyConfigJson(const String& json, String& errorMsg) {
    ...
    if (doc.containsKey("web_user")) prefs.putString("web_user", doc["web_user"].as<String>());
    if (doc.containsKey("web_pass")) prefs.putString("web_pass", doc["web_pass"].as<String>());
    prefs.end();
    ...
    if (doc.containsKey("wifi_ssid")) {
        ...
        // The user might need to re-configure WiFi via AP mode if password was lost.
    }
