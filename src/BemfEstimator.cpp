...
void BemfEstimator::setMotorParams(float rArmature, int poles) {
    if (rArmature > 0.0f) _rArmature = rArmature;
    if (poles > 0) _poles = poles;
}
...
void BemfEstimator::updateRippleFreq(float freqHz) {
    _rippleFreq = freqHz;
}
...
void BemfEstimator::calculateEstimate() {
    ...
    if (rippleRpm > 600.0f) {
        ...
        // Crossover region
        if (_useRipple) _estimatedRpm = rippleRpm;
        else _estimatedRpm = estimatedRpmFromBemf;
    }
}

float BemfEstimator::getEstimatedRpm() const {
    return _estimatedRpm;
}

float BemfEstimator::getBemfVoltage() const {
    return _vBemf;
}
