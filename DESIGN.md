...
We must track the _actual speed_ of the motor. When speed drops, we add torque. The millisecond the bind clears and the motor attempts to surge, the speed measurement spikes, allowing the PI loop to instantly brake the motor. This yields a flawless, ultra-low-speed crawl.
...
$$ V*{bemf} = V*{applied} - (I*{motor} \times R*{armature}) $$
