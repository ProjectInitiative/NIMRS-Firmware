...
Updates web interface credentials. Empty strings disable auth.
...
| action  | String | Action to perform ("stop", "toggle_lights", "set_function", "set_speed", "set_direction", "set_log_level"). |
...
| Name  | Type | Description                                              |
| ----- | ---- | -------------------------------------------------------- |
| plain | JSON | JSON object where keys are CV IDs and values are values. |
