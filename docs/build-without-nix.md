...
-   **Partition Scheme:** You must select a scheme that supports at least **2MB** for the application.
    -   *Note: The project uses a custom `partitions.csv`. To use it in Arduino IDE, you may need to place it in the variant folder or select "Custom" if your board definition supports it. Otherwise, select "8M with SPIFFS" (ensure APP is >1.8MB).*
