...
struct JsonVariant {
    ...
    template<typename T>
    void operator=(T v) { val = JsonVariant(v).val; }
...
inline void serializeJson(const JsonDocument& doc, String& out) { out = "{\"mock\":true}"; }
...
inline DeserializationError deserializeJson(JsonDocument& doc, const String& in) {
    ...
    size_t pos = s.find('{');
    if (pos == std::string::npos) return DeserializationError::Ok;
    pos++;
    ...
    while (true) {
        ...
        if (pos == std::string::npos || s[pos] == '}') break;
        pos++;
    }
