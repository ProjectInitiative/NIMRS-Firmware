...
// This is a common technique to test static/internal functions or avoid linking
// issues with mocks
...
void test_notifyDccSpeed_basic() {
  ...
  if (state.speed != 50) {
    std::cout << "FAILED: Speed should be 50, got " << (int)state.speed
              << std::endl;
...
void test_notifyDccSpeed_hysteresis() {
  ...
  if (state.speedSource != SOURCE_WEB) {
    std::cout << "FAILED: Should remain SOURCE_WEB for small change"
              << std::endl;
...
void test_notifyDccSpeed_override() {
  ...
  if (state.speedSource != SOURCE_DCC) {
    std::cout << "FAILED: Should switch to SOURCE_DCC for large change"
              << std::endl;
    assert(state.speedSource == SOURCE_DCC);
  }
  if (state.speed != 60) {
...
void test_notifyDccSpeed_direction_change() {
  ...
  if (state.speedSource != SOURCE_DCC) {
    std::cout << "FAILED: Should switch to SOURCE_DCC for direction change"
              << std::endl;
    assert(state.speedSource == SOURCE_DCC);
  }
...
void test_notifyDccSpeed_speed_steps() {
  ...
  // Speed 1 -> 0 (E-Stop usually, mapped to 0 here based on code: if (Speed >
  // 1) target=Speed else 0)
