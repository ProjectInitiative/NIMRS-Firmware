...
// But we compile from root, so includes in Logger.cpp (like #include
// "Logger.h") will look in -Isrc This include here is textual inclusion.
...
int main() {
  ...
  // Expected format: "[1000] Test Message" (millis is mocked to 1000 in
  // Arduino.h)

  // Check if it contains the message and timestamp format
  // Note: Arduino String implementation in mocks/Arduino.h might behave
  // slightly differently but basic operators should work. However, the mock
  // String inherits std::string.

  if (lastLog.find("[1000] Test Message") == std::string::npos) {
    std::cerr << "Error: Log format incorrect. Got: " << lastLog.c_str()
              << std::endl;
    return 1;
  }
  ...
  std::cout << "Time taken for 100,000 logs: " << diff.count() << " s"
            << std::endl;
  std::cout << "Average time per log: " << (diff.count() / 100000.0) * 1e6
            << " us" << std::endl;
